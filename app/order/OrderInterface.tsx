  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div className={`sticky top-20 z-40 transition-all duration-300 ${isScrolled ? 'py-2' : 'py-6'}`}>
        <div className="bg-white/90 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/20 p-2 md:p-3 flex justify-center max-w-2xl mx-auto px-4">
          <div className="relative w-full group">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 outline-none transition-all placeholder:text-slate-400 text-slate-900 font-medium"
      ...
      {/* --- PRODUCT GRID --- */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8 pb-48">
          ...
          {filteredProducts.map((product, index) => {
             ...
             return (
              <motion.div
                ...
                className={`group bg-white rounded-3xl border overflow-hidden transition-all duration-300 flex flex-col ${
              ...
              >
                {/* Image */}
                <div className="relative aspect-[4/3] p-6 bg-slate-50 flex items-center justify-center overflow-hidden">
                  {index % 3 === 0 && (
                  ...
                  <Link href={`/products/${product.slug}`} className="relative w-full h-full">
                    <Image
                ...
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  <div className="mt-auto pt-4 border-t border-slate-50 flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`h-12 px-6 rounded-xl font-bold text-sm active:scale-95 transition-all shadow-lg flex items-center gap-2 ${
                        alreadyAdded 
      ...
      <AnimatePresence>
        {hasMounted && cartCount > 0 && (
          <motion.div
            ...
            className="fixed bottom-6 left-4 right-4 md:left-1/2 md:-translate-x-1/2 md:w-full md:max-w-2xl z-50"
          >
            <div className="bg-slate-900 text-white p-3 pr-4 rounded-2xl shadow-2xl shadow-teal-900/50 flex items-center justify-between border border-slate-700/50 backdrop-blur-md">
              <div className="flex items-center gap-4">
                <div className="bg-teal-500 h-12 w-12 rounded-xl flex items-center justify-center relative shrink-0">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold leading-none">Rs. {cartTotal.toLocaleString()}</p>
                </div>
              </div>

              <Link href="/checkout">
                <button className="bg-white text-slate-900 px-8 py-3 rounded-xl font-bold text-sm hover:bg-teal-50 transition-colors shadow-lg active:scale-95 flex items-center gap-2">
                  Checkout Now <span className="text-lg">→</span>
                </button>
              </Link>
            </div>
          </motion.div>
