    ...
    <main className="min-h-screen bg-[#fbfcfa] text-slate-900 font-sans overflow-hidden">
      ...
      <section className="relative pt-28 md:pt-32 pb-16 md:pb-20 px-4 md:px-8 lg:px-16 max-w-7xl mx-auto grid md:grid-cols-2 gap-8 md:gap-12 items-center">
        ...
        >
          ...
          <p className="text-sm md:text-xl text-[#1e9b85] font-bold">Advanced Mass & Appetite Stimulator</p>

          <div className="flex items-center gap-2 text-amber-500">
            {[1, 2, 3, 4, 5].map(i => <Star key={i} size={16} className="md:w-[18px] md:h-[18px]" fill="currentColor" />)}
            <span className="text-slate-500 text-xs md:text-sm ml-2 font-medium">(214 Reviews)</span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 md:gap-6 pt-5 w-full max-w-md">
            {[ { icon: <Leaf/>, label: "100% Herbal" }, { icon: <Utensils/>, label: "Eat More" }, { icon: <ShieldCheck/>, label: "Clinical" } ].map((item, i) => (
        ...
        <div className="relative h-[350px] md:h-[600px] w-full flex justify-center items-center mt-8 md:mt-0">
          ...
          >
            <Image src={GainForteImg} alt="GainForte Syrup" fill className="object-cover p-2 md:p-0" priority />
            <motion.div animate={{ y: [0, -10, 0] }} transition={{ repeat: Infinity, duration: 3 }} className="absolute top-4 md:top-6 right-4 md:right-6 bg-white/90 backdrop-blur-md rounded-xl md:rounded-2xl p-3 md:p-4 shadow-xl border border-slate-100">
              <Activity size={20} className="md:w-6 md:h-6 text-[#df2c25] mb-1" />
              <div className="text-[8px] md:text-[10px] font-bold uppercase text-slate-400">Metabolic</div>
              <div className="text-xs md:text-sm font-black text-slate-900">Boost</div>
            </motion.div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-16 md:py-24 bg-white px-4 relative overflow-hidden">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-10 md:mb-16">
            ...
            <h2 className="text-3xl md:text-5xl font-extrabold uppercase text-slate-900">Nature's <span className="text-[#1e9b85]">Growth Force</span></h2>
          </div>
          ...
          <div className="relative flex flex-col md:flex-row items-center justify-center gap-10 md:gap-24">
            <div className="flex flex-col space-y-8 md:space-y-12 text-center md:text-right w-full">
              <div>
                <h4 className="font-extrabold text-lg md:text-xl uppercase tracking-wider mb-2 text-[#1e9b85]">Ashwagandha</h4>
                <p className="text-xs md:text-sm text-slate-500 max-w-[240px] mx-auto md:ml-auto md:mr-0">Reduces metabolic stress and naturally supports healthy weight gain.</p>
              </div>
            </div>
            ...
            <div className="relative w-48 h-64 md:w-64 md:h-80 shrink-0">
              ...
              <Image src={GainForteImg} alt="GainForte" fill className="object-cover rounded-3xl relative z-10 drop-shadow-xl border-4 border-white" />
            </div>

            <div className="flex flex-col space-y-8 md:space-y-12 text-center md:text-left w-full">
              <div>
                <h4 className="font-extrabold text-lg md:text-xl uppercase tracking-wider mb-2 text-[#1e9b85]">Shatavari</h4>
                <p className="text-xs md:text-sm text-slate-500 max-w-[240px] mx-auto md:ml-0 md:mr-auto">Improves nutrient absorption and gently stimulates hunger cues.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-16 md:py-24 bg-white px-4">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl md:text-5xl font-extrabold text-center mb-12 md:mb-16 uppercase text-slate-900">Why choose <span className="text-[#1e9b85]">GainForte</span></h2>
          
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
            {[ { icon: <Utensils/>, title: "Appetite Boost" }, { icon: <Activity/>, title: "Muscle Mass" }, { icon: <Zap/>, title: "More Energy" }, { icon: <ShieldCheck/>, title: "100% Clinical" } ].map((benefit, i) => (
      ...
      <section className="py-16 md:py-24 bg-[#fbfcfa] px-4 border-t border-slate-200">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 md:gap-16">
          ...
          <div>
            ...
            <h2 className="text-3xl md:text-4xl font-extrabold uppercase tracking-tight mb-8 md:mb-10 text-slate-900">User Stories</h2>
            <div className="space-y-3 md:space-y-4">
              {[ { n: "Bilal K.", r: 5, t: "Amazing results. Gained 4kg in a month and my energy levels are higher than ever." }, { n: "Sarah M.", r: 5, t: "Finally found a syrup that works without feeling heavy. My appetite is back!" } ].map((rev, i) => (
      ...
      <footer className="py-20 md:py-24 bg-[#0a251e] text-center px-4 relative overflow-hidden">
        ...
        <div className="relative z-10">
            <h2 className="text-3xl md:text-6xl font-extrabold text-white mb-6 md:mb-8 tracking-tight">TRANSFORM <br/>YOUR BODY TODAY</h2>
